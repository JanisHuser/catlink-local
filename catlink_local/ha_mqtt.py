    ...
    return [
        ...
        (
            f"{DISCOVERY_PREFIX}/sensor/catlink_{mid}/last_entry/config",
            {
                ...
                "state_topic": st,
                "value_template": "{{ value_json.last_entry_at | default('', true) }}",
        ...
        # Diagnostics: technical/reverse-engineering aids, tucked under HA's
        # diagnostic category rather than the main controls.
        (
            f"{DISCOVERY_PREFIX}/sensor/catlink_{mid}/last_event/config",
            {
                "name": "Last event",
                "unique_id": f"catlink_{mid}_last_event",
                "state_topic": st,
                "value_template": "{{ value_json.last_event_at | default('', true) }}",
                "device_class": "timestamp",
                "entity_category": "diagnostic",
        ...
        (
            f"{DISCOVERY_PREFIX}/sensor/catlink_{mid}/raw_status/config",
            {
                "name": "Raw status",
                "unique_id": f"catlink_{mid}_raw_status",
                "state_topic": st,
                "value_template": "{{ value_json.last_status_hex }}",
                "entity_category": "diagnostic",
...
def state_payload(record: Any) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "last_status_hex": state.get("last_status_hex"),
        # common
